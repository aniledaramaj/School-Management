...
def custom_logout(request):
    ...
    return redirect('choose_login')  # Redirect to your app's dashboard after logout
...
def register_student(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.is_student = True  # Set the is_student field to True for students
            user.save()
            login(request, user)
...
def student_dashboard(request):
    if request.user.is_authenticated:
        ...
        return redirect('login_student')  # Redirect to student login page
...
def serve_lesson_file(request, subject_id, leksioni):
    # Retrieve the Kurset object
    kurset = get_object_or_404(Kurset, subject_id=subject_id, leksioni=leksioni)

    # Construct the correct relative file path within the 'pdfs' directory
